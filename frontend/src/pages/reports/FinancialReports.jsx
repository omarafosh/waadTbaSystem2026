 ...
 * 
 * Permissions: INSURANCE_ADMIN, REVIEWER, SUPER_ADMIN (NOT PROVIDER)
 */
const FinancialReports = () => {
  ...
  const canFilterByProvider = useMemo(() => {
    const allowedRoles = [ROLES.INSURANCE_ADMIN, ROLES.REVIEWER, ROLES.SUPER_ADMIN];
  ...
  useEffect(() => {
    fetchData();
  }, [activeTab, selectedEmployer, selectedProviderId, page, rowsPerPage, dateFrom, dateTo, status]);

  // Reset pagination when filters change
  useEffect(() => {
    setPage(0);
  }, [selectedEmployer, selectedProviderId, status, dateFrom, dateTo]);
  ...
  const fetchFinancialSummary = async () => {
    // ══════════════════════════════════════════════════════════════════════════
    // STEP 1: Fetch authoritative totals from backend (SINGLE SOURCE OF TRUTH)
    // ══════════════════════════════════════════════════════════════════════════
    ...
    const backendSummary = summaryResponse || {};

    // ══════════════════════════════════════════════════════════════════════════
    // STEP 2: Set KPIs from backend (NO client-side calculations!)
    // ══════════════════════════════════════════════════════════════════════════
    setSummaryData({
      // Amounts - directly from database SUM() queries
      totalClaimsAmount: backendSummary.totalRequestedAmount || 0,
      totalApprovedAmount: backendSummary.totalApprovedAmount || 0,
      totalPaidAmount: backendSummary.totalSettledAmount || 0,
      outstandingAmount: backendSummary.outstandingAmount || 0,
      totalPatientCoPay: backendSummary.totalPatientCoPay || 0,
      totalNetProviderAmount: backendSummary.totalNetProviderAmount || 0,
      
      // Counts - directly from database COUNT() queries
      claimsCount: backendSummary.totalClaimsCount || 0,
      approvedCount: backendSummary.approvedClaimsCount || 0,
      settledCount: backendSummary.settledClaimsCount || 0,
      pendingCount: backendSummary.pendingClaimsCount || 0,
      underReviewCount: backendSummary.underReviewClaimsCount || 0,
      rejectedCount: backendSummary.rejectedClaimsCount || 0,
      
      // Breakdowns
      providerSummaries: backendSummary.providerSummaries || [],
      statusSummaries: backendSummary.statusSummaries || [],
      employerSummaries: backendSummary.employerSummaries || []
    });

    // ══════════════════════════════════════════════════════════════════════════
    // STEP 3: Fetch sample claims for preview table (limited to 10)
    // ══════════════════════════════════════════════════════════════════════════
    ...
    const previewClaims = allClaimsResponse.data?.items || [];
    setClaims(previewClaims);
    setTotalRows(backendSummary.totalClaimsCount || 0);
  };

  const fetchInvoicesReport = async () => {
    // Fetch settled claims grouped by employer
  ...
  const fetchPaymentsReport = async () => {
    // Fetch settled claims with payment info
    const response = await claimsService.getAll({
      employerId: selectedEmployer,
      providerId: selectedProviderId || undefined,
      page: page + 1,
    ...
    const payments = (response.data?.items || [])
      .filter(c => c.status === 'SETTLED' && c.paymentReference)
      .map(claim => ({
        id: claim.id,
  ...
  const getColumns = () => {
    switch (activeTab) {
      case 0: return summaryColumns;
      case 1: return invoicesColumns;
      case 2: return paymentsColumns;
      case 3: return settlementsColumns;
      default: return summaryColumns;
    }
  };
    ...
    >
      <Box>
        <ModernPageHeader
          ...
          subtitle="تقارير شاملة للمطالبات والتسويات والمدفوعات"
          icon={<MoneyIcon />}
        />
              ...
              <Grid container spacing={3} sx={{ mb: 3 }}>
                <Grid item xs={12} sm={6} md={3}>
                  <Card elevation={2} sx={{ background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)', color: 'white' }}>
                    <CardContent>
                      <Typography variant="h6" gutterBottom>إجمالي المطالبات</Typography>
                ...
                <Grid item xs={12} sm={6} md={3}>
                  <Card elevation={2} sx={{ background: 'linear-gradient(135deg, #4facfe 0%, #00f2fe 100%)', color: 'white' }}>
                    <CardContent>
                      <Typography variant="h6" gutterBottom>إجمالي المدفوع</Typography>
                      <Typography variant="h4" fontWeight={700}>
                ...
                <Grid item xs={12} sm={6} md={3}>
                  <Card elevation={2} sx={{ background: 'linear-gradient(135deg, #fa709a 0%, #fee140 100%)', color: 'white' }}>
                    <CardContent>
                      <Typography variant="h6" gutterBottom>المبلغ المعلق</Typography>
                      <Typography variant="h4" fontWeight={700}>
          ...
          >
            <Box ref={printRef} sx={{ p: 2 }}>
              ...
              <Box sx={{ textAlign: 'center', mb: 3, pb: 2, borderBottom: '2px solid #1976d2' }}>
                ...
                {selectedEmployer && (
                  <Typography variant="body2" color="text.secondary">
                    الشريك: {selectedEmployer}
                  </Typography>
                )}
                {selectedProviderId && (
                  <Typography variant="body2" color="text.secondary">
                    مقدم الخدمة: {providers.find(p => p.id === selectedProviderId)?.name || selectedProviderId}
                  </Typography>
                )}
                <Typography variant="body2" color="text.secondary">
                ...
                <Grid container spacing={2} sx={{ mb: 3 }}>
                  <Grid item xs={3}>
                    <Paper sx={{ p: 2, textAlign: 'center' }}>
                      ...
                      <Typography variant="h6" fontWeight="bold">{formatAmount(summaryData.totalClaimsAmount)}</Typography>
                    </Paper>
                  </Grid>
                  <Grid item xs={3}>
                    <Paper sx={{ p: 2, textAlign: 'center' }}>
                      <Typography variant="caption">إجمالي المعتمد</Typography>
                      <Typography variant="h6" fontWeight="bold">{formatAmount(summaryData.totalApprovedAmount)}</Typography>
                    </Paper>
                  </Grid>
                  <Grid item xs={3}>
                    <Paper sx={{ p: 2, textAlign: 'center' }}>
                      <Typography variant="caption">إجمالي المدفوع</Typography>
                      <Typography variant="h6" fontWeight="bold">{formatAmount(summaryData.totalPaidAmount)}</Typography>
                    </Paper>
                  </Grid>
                  <Grid item xs={3}>
                    <Paper sx={{ p: 2, textAlign: 'center' }}>
                      <Typography variant="caption">المعلق</Typography>
                      <Typography variant="h6" fontWeight="bold">{formatAmount(summaryData.outstandingAmount)}</Typography>
                    </Paper>
                  </Grid>
                </Grid>
              )}
