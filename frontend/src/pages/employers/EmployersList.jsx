...
const EmployersList = () => {
        ...
        cell: ({ row }) => (
          <Stack direction="row" spacing={1} alignItems="center">
            ...
            {row.original?.archived && (
              <Chip label="مؤرشف" size="small" color="warning" variant="outlined" />
            )}
          </Stack>
        )
      },
              ...
              <Tooltip title="استعادة من الأرشيف">
                <PermissionGuard requires="employers.delete">
                  <IconButton
                    size="small"
                    color="success"
                    onClick={() => handleRestore(row.original?.id, row.original?.nameAr || row.original?.code)}
                  >
              ...
              <Tooltip title="أرشفة">
                <PermissionGuard requires="employers.delete">
                  <IconButton
                    size="small"
                    color="warning"
                    onClick={() => handleArchive(row.original?.id, row.original?.nameAr || row.original?.code)}
                  >
  ...
  return (
    <Box>
      <PermissionGuard requires="employers.view">
        <UnifiedPageHeader
          title="الشركاء"
