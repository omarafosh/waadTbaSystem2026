...
export const EmployerFilterProvider = ({ children }) => {
  ...
  const setEmployer = useCallback((partner) => {
    ...
    setSelectedEmployer({
      id: partner.id,
      label: partner.label || partner.name || partner.nameAr || partner.nameArabic,
      name: partner.label || partner.name || partner.nameAr || partner.nameArabic,
      code: partner.code
    });
    ...
    try {
      ...
      localStorage.setItem('tba_selected_partner', JSON.stringify({
        id: partner.id,
        label: partner.label || partner.name || partner.nameAr || partner.nameArabic,
        name: partner.label || partner.name || partner.nameAr || partner.nameArabic,
        code: partner.code
  ...
  useEffect(() => {
    try {
      ...
      if (savedId && savedData) {
        ...
        setSelectedEmployer({
          id: parsed.id,
          label: parsed.label || parsed.name || parsed.nameAr || parsed.nameArabic,
          name: parsed.label || parsed.name || parsed.nameAr || parsed.nameArabic,
          code: parsed.code
        });
      }
    } catch (error) {
