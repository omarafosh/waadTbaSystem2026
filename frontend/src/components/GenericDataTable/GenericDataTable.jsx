...
const GenericTableRow = memo(({ row, onRowClick, cellPadding }) => (
  <TableRow
    hover
...
const GenericDataTable = memo(
  ({
    ...
    // Stable reference for onRowClick to prevent unnecessary re-renders of GenericTableRow
      ...
      return (
        <TableBody>
          {table.getRowModel().rows.map((row) => (
            <GenericTableRow key={row.id} row={row} onRowClick={onRowClick ? handleRowClick : undefined} cellPadding={cellPadding} />
          ))}
        </TableBody>
      );
    };
