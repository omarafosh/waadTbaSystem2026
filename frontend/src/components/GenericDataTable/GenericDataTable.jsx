...
import { useMemo, Fragment, memo, useRef, useCallback, useEffect } from 'react';
...
/**
 * GenericDataTableRow - Optimized Row Component
 *
 * Prevents unnecessary re-renders when parent re-renders but row data is stable.
 */
const GenericDataTableRow = memo(
  ({ row, onClick, cellPadding, enableRowClick, columns }) => {
    return (
      <TableRow
        hover
        onClick={() => enableRowClick && onClick(row)}
        sx={{
          cursor: enableRowClick ? 'pointer' : 'default',
          '&:hover': {
            backgroundColor: enableRowClick ? 'action.hover' : 'inherit'
          }
        }}
      >
  ...
  (prevProps, nextProps) => {
    ...
    return (
      ...
      prevProps.enableRowClick === nextProps.enableRowClick &&
...
const GenericDataTable = memo(
  ({
    ...
    // Optimization: Stabilize onRowClick to prevent row re-renders
