...
public class ClaimMapper {
    ...
    public Claim toEntity(ClaimCreateDto dto) {
        ...
        // ⚡ Bolt: Batch fetch MedicalServices to prevent N+1 queries in the loop
        List<Long> medicalServiceIds = dto.getLines().stream()
                .map(ClaimLineDto::getMedicalServiceId)
                .collect(Collectors.toList());

        if (medicalServiceIds.contains(null)) {
        ...
        for (ClaimLineDto lineDto : dto.getLines()) {
            // Fetch MedicalService from Map (O(1) lookup instead of DB query)
