...
public interface ProviderContractRepository extends JpaRepository<ProviderContract, Long> {
       ...
       /**
        * Find valid active contracts for a provider and employer on a specific date.
        * Checks:
        * 1. Provider ID match
        * 2. Employer ID match
        * 3. Status is ACTIVE
        * 4. Record is active (not soft deleted)
        * 5. Date is within [startDate, endDate]
        */
       @Query("SELECT c FROM ModernProviderContract c WHERE c.provider.id = :providerId AND c.employer.id = :employerId AND c.status = 'ACTIVE' AND c.active = true AND (c.endDate IS NULL OR c.endDate >= :serviceDate) AND c.startDate <= :serviceDate")
    ...
    Page<ProviderContract> findByProviderId(Long providerId, Pageable pageable);
       ...
       /**
        * Search contracts by provider name (Arabic or English) or contract code
        */
       @Query("SELECT c FROM ModernProviderContract c " +
                     "WHERE c.active = true " +
        ...
        * Returns: [id, contractCode, provider.nameArabic, status, createdAt]
        */
       @Query("SELECT c.id, " +
                     "c.contractCode, " +
