...
public class PreAuthorizationService {
    ...
    public PreAuthorizationResponseDto checkValidity(Long memberId, String serviceCode) {
        ...
        // Find approved and valid pre-authorizations for this member and service using database query
        List<PreAuthorization> validPreAuths = preAuthorizationRepository.findValidPreAuthsForMemberAndService(
                memberId, serviceCode, LocalDate.now());
        ...
        // Return the most recent valid one (query already orders by createdAt DESC, so index 0 is the most recent)
        PreAuthorization preAuth = validPreAuths.get(0);
