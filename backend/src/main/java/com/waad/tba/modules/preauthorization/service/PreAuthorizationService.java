...
public class PreAuthorizationService {
    ...
    public PreAuthorizationResponseDto checkValidity(Long memberId, String serviceCode) {
        ...
        // Find approved and valid pre-authorizations for this member and service via database query
        // Order by createdAt DESC handles returning the most recent one
        List<PreAuthorization> validPreAuths = preAuthorizationRepository.findValidPreAuthorizationsByMemberAndService(
                memberId,
                serviceCode,
        ...
        // Return the most recent valid one (already ordered by createdAt DESC in the repository)
        PreAuthorization preAuth = validPreAuths.get(0);
