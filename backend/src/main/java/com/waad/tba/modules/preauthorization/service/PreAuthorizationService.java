...
public class PreAuthorizationService {
    ...
    public PreAuthorizationResponseDto checkValidity(Long memberId, String serviceCode) {
        ...
        // Bolt ⚡: Optimization - Replace full table scan & in-memory filter with direct DB query
        List<PreAuthorization> validPreAuths = preAuthorizationRepository.findValidPreAuthForMemberAndService(
                memberId,
                serviceCode,
                LocalDate.now()
        );
        ...
        // Return the most recent valid one (already sorted by query DESC)
        PreAuthorization preAuth = validPreAuths.get(0);
