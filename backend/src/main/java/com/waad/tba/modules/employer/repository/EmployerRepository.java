...
public interface EmployerRepository extends JpaRepository<Employer, Long> {
    ...
    /**
     * Find employer by name (Arabic only, case-insensitive)
     */
    @Query("SELECT e FROM Employer e WHERE LOWER(e.nameAr) = LOWER(:name)")
    Optional<Employer> findByNameIgnoreCase(@Param("name") String name);

    /**
     * Find employers by name containing (partial match for import - searches Arabic
     * only)
     */
    @Query("SELECT e FROM Employer e WHERE LOWER(e.nameAr) LIKE LOWER(CONCAT('%', :name, '%'))")
