...
public class RoleManagementService {
    ...
    public List<RoleViewDto> getAllRoles() {
        ...
        // Batch fetch all role counts to prevent N+1 queries
        List<Long> roleIds = roles.stream().map(Role::getId).collect(Collectors.toList());
        Map<Long, Long> countsMap = new HashMap<>();
        if (!roleIds.isEmpty()) {
            countsMap = userRepository.countUsersGroupedByRoleIds(roleIds).stream()
                .collect(Collectors.toMap(
                    UserRepository.UserCountByRole::getRoleId,
                    UserRepository.UserCountByRole::getUserCount
                ));
        }

        final Map<Long, Long> finalCountsMap = countsMap;

        return roles.stream()
                .map(role -> toViewDtoWithCount(role, finalCountsMap.getOrDefault(role.getId(), 0L).intValue()))
