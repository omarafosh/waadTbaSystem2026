...
public class RoleManagementService {
    ...
    public List<RoleViewDto> getAllRoles() {
        ...
        java.util.Map<Long, Integer> roleUserCountMap = new java.util.HashMap<>();
        for (Object[] row : userCounts) {
            Long roleId = (Long) row[0];
            Integer count = ((Number) row[1]).intValue();
            roleUserCountMap.put(roleId, count);
        }

        return roles.stream()
                .map(role -> {
                    int count = roleUserCountMap.getOrDefault(role.getId(), 0);
                    return toViewDto(role, count);
                })
