...
public class RoleManagementService {
    ...
    public List<RoleViewDto> getAllRoles() {
        ...
        // Performance Optimization: Prevent N+1 query and O(N) memory load by batch fetching counts in a single query
        // Empty check prevents SQL syntax errors if no roles exist
        Map<Long, Long> userCounts = roleIds.isEmpty() ? Map.of() : userRepository.countUsersByRoleIds(roleIds).stream()
        ...
        return roles.stream()
                .map(role -> toViewDto(role, userCounts.getOrDefault(role.getId(), 0L).intValue()))
    ...
    private RoleViewDto toViewDto(Role role) {
        return toViewDto(role, (int) countUsersWithRole(role.getId()));
    }
