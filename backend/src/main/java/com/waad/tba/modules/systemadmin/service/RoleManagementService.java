...
public class RoleManagementService {
    ...
    public List<RoleViewDto> getAllRoles() {
        ...
        if (!roleIds.isEmpty()) {
            ...
            for (Object[] countRow : counts) {
                countsMap.put((Long) countRow[0], (Long) countRow[1]);
            }
        }

        return roles.stream()
                .map(role -> toViewDtoWithCount(role, countsMap.getOrDefault(role.getId(), 0L).intValue()))
    ...
    private RoleViewDto toViewDtoWithCount(Role role, int count) {
