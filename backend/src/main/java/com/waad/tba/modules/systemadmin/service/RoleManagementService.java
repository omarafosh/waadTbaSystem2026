...
public class RoleManagementService {
    ...
    public List<RoleViewDto> getAllRoles() {
        ...
        return mapRolesToViewDtos(roles);
    }
    ...
    private List<RoleViewDto> mapRolesToViewDtos(List<Role> roles) {
        ...
        // BOLT OPTIMIZATION: Eliminate N+1 queries by bulk fetching counts via single GROUP BY query
        List<Long> roleIds = roles.stream().map(Role::getId).collect(Collectors.toList());
        List<Object[]> counts = userRepository.countUsersByRoleIds(roleIds);

        // Map the results in-memory for O(1) lookup
        ...
        return roles.stream()
                .map(role -> toViewDto(role, roleUserCounts.getOrDefault(role.getId(), 0L)))
                .collect(Collectors.toList());
    }
}
