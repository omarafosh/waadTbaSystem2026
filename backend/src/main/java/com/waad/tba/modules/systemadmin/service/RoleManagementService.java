...
public class RoleManagementService {
    ...
    public List<RoleViewDto> getAllRoles() {
        ...
        // Performance Optimization: Prevent SQL syntax error on empty IN clause
        if (roles.isEmpty()) {
            return List.of();
        }
        ...
        // Performance Optimization: Bulk fetch user counts to avoid N+1 queries during mapping
        Map<Long, Long> roleCounts = userRepository.countUsersByRoleIds(roleIds).stream()
        ...
        return roles.stream()
                .map(role -> toViewDto(role, roleCounts.getOrDefault(role.getId(), 0L).intValue()))
