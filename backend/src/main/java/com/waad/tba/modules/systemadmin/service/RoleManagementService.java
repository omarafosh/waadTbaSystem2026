...
public class RoleManagementService {
    ...
    public List<RoleViewDto> getAllRoles() {
        ...
        java.util.Map<Long, Long> userCounts = getUserCountsForRoles(roleIds);

        return roles.stream()
                .map(role -> toViewDtoWithCount(role, userCounts.getOrDefault(role.getId(), 0L).intValue()))
    ...
    private java.util.Map<Long, Long> getUserCountsForRoles(List<Long> roleIds) {
        if (roleIds.isEmpty()) return java.util.Collections.emptyMap();
        List<Object[]> counts = userRepository.countUsersGroupedByRoleIds(roleIds);
