...
public interface VisitRepository extends JpaRepository<Visit, Long> {
     ...
     */
    @Query(value = "SELECT v FROM Visit v LEFT JOIN FETCH v.member m LEFT JOIN FETCH m.employerOrganization " +
           "WHERE v.active = true " +
           "AND (:providerId IS NULL OR v.providerId = :providerId) " +
           "AND (:memberId IS NULL OR m.id = :memberId) " +
           "AND (:memberName IS NULL OR :memberName = '' OR LOWER(m.fullName) LIKE LOWER(CONCAT('%', :memberName, '%')) OR m.cardNumber LIKE CONCAT('%', :memberName, '%') OR m.nationalNumber LIKE CONCAT('%', :memberName, '%')) " +
